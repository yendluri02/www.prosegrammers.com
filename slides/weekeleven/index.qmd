...
{{< iconify fa6-solid lightbulb >}} Prosegrammers use searching and sorting to
make documents more accessible and useful!
...
- {{< iconify fa6-solid microscope >}} **How do we measure algorithm efficiency?**
...
- **Linear search**: check every item sequentially ($O(n)$)
- **Binary search**: divide and conquer approach ($O(\log n)$)
...
- Python's `sorted()` uses efficient timsort ($O(n \log n)$)
...
- Both use timsort algorithm ($O(n \log n)$ average case)
...
{{< iconify fa6-solid lightbulb >}} Understand algorithm complexity to make
smart choices for your document engineering projects!
